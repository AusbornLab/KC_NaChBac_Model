...
#define initmodel initmodel__nat
#define _net_receive _net_receive__nat
#define _net_init _net_init__nat
#define nrn_state_launcher nrn_state_nat_launcher
#define nrn_cur_launcher nrn_cur_nat_launcher
#define nrn_jacob_launcher nrn_jacob_nat_launcher 
#define states states_nat 
...
#define t _nt->_t
#define dt _nt->_dt
#define natgmax _p[0*_STRIDE]
#define ina _p[1*_STRIDE]
#define ina2nat _p[2*_STRIDE]
...
#endif /*BBCORE*/
#define tauh tauh_nat
#define taum taum_nat
 #pragma acc routine seq
 inline double tauh( _threadargsprotocomma_ double );
 #pragma acc routine seq
 inline double taum( _threadargsprotocomma_ double );
...
#define _slist1 _slist1_nat
int* _slist1;
#pragma acc declare create(_slist1)

#define _dlist1 _dlist1_nat
int* _dlist1;
...
/*CVODE*/
 static int _ode_spec1 (_threadargsproto_) {int _reset = 0; {
    Dm = ( ( 1.0 / ( 1.0 + exp ( 0.1121 * ( - 29.13 - v ) ) ) ) - m ) / taum ( _threadargscomma_ v ) ;
   Dh = ( ( 1.0 / ( 1.0 + exp ( - 0.2 * ( - 47.0 - v ) ) ) ) - h ) / tauh ( _threadargscomma_ v ) ;
    }
 return _reset;
}
 static int _ode_matsol1 (_threadargsproto_) {
 ...
 static int states (_threadargsproto_) { {
     m = m + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / taum ( _threadargscomma_ v ))))*(- ( ( ( ( ( 1.0 ) / ( 1.0 + exp ( 0.1121 * ( - 29.13 - v ) ) ) ) ) ) / taum ( _threadargscomma_ v ) ) / ( ( ( ( - 1.0) ) ) / taum ( _threadargscomma_ v ) ) - m) ;
    h = h + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tauh ( _threadargscomma_ v ))))*(- ( ( ( ( ( 1.0 ) / ( 1.0 + exp ( - 0.2 * ( - 47.0 - v ) ) ) ) ) ) / tauh ( _threadargscomma_ v ) ) / ( ( ( ( - 1.0) ) ) / tauh ( _threadargscomma_ v ) ) - h) ;
    }
  return 0;
}
...
double taum ( _threadargsprotocomma_ double _lVm ) {
   double _ltaum;
  _ltaum = 0.1270 + 3.434 / ( 1.0 + exp ( ( v + 45.35 ) / 5.98 ) ) ;
...
static void _hoc_taum(void) {
  ...
  _nt = nrn_threads;
 _r =  taum ( _threadargs_, *getarg(1) ;
 hoc_retpushx(_r);
}
 
#endif /*BBCORE*/
 
double tauh ( _threadargsprotocomma_ double _lVm ) {
   double _ltauh;
  _ltauh = 0.36 + exp ( ( v + 20.65 ) / ( - 10.47 ) ) ;
