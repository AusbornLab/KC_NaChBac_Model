...
#define nrn_init _nrn_init__ks_gunay
#define nrn_cur _nrn_cur__ks_gunay
#define _nrn_current _nrn_current__ks_gunay
#define nrn_jacob _nrn_jacob__ks_gunay
#define nrn_state _nrn_state__ks_gunay
#define initmodel initmodel__ks_gunay
#define _net_receive _net_receive__ks_gunay
#define _net_init _net_init__ks_gunay
#define nrn_state_launcher nrn_state_ks_gunay_launcher
#define nrn_cur_launcher nrn_cur_ks_gunay_launcher
#define nrn_jacob_launcher nrn_jacob_ks_gunay_launcher 
#define states states_ks_gunay 
...
#define t _nt->_t
#define dt _nt->_dt
#define ksgmax _p[0*_STRIDE]
#define ik _p[1*_STRIDE]
#define ik2 _p[2*_STRIDE]
...
#define h _mlhh
 ...
 "setdata_ks_gunay", _hoc_setdata,
 "taum_ks_gunay", _hoc_taum,
 0, 0
};
 
#endif /*BBCORE*/
#define taum taum_ks_gunay
...
#define _slist1 _slist1_ks_gunay
int* _slist1;
#pragma acc declare create(_slist1)

#define _dlist1 _dlist1_ks_gunay
int* _dlist1;
...
/*CVODE*/
 static int _ode_spec1 (_threadargsproto_) {int _reset = 0; {
   Dm = ( ( 1.0 / ( 1.0 + exp ( 0.0502 * ( - 12.85 - v ) ) ) ) - m ) / taum ( _threadargscomma_ v ) ;
   }
 return _reset;
}
 static int _ode_matsol1 (_threadargsproto_) {
 ...
 static int states (_threadargsproto_) { {
    m = m + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / taum ( _threadargscomma_ v ))))*(- ( ( ( ( ( 1.0 ) / ( 1.0 + exp ( 0.0502 * ( - 12.85 - v ) ) ) ) ) ) / taum ( _threadargscomma_ v ) ) / ( ( ( ( - 1.0) ) ) / taum ( _threadargscomma_ v ) ) - m) ;
   }
  return 0;
}
...
double taum ( _threadargsprotocomma_ double _lVm ) {
   double _ltaum;
 _ltaum = 2.03 + 1.96 / ( 1.0 + exp ( ( _lVm - 30.83 ) / 3.12 ) ) ;
